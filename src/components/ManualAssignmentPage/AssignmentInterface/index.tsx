...
interface AssignmentInterfaceProps {
    pendingOrders?: IDispatcherOrder[];
    isLoadingOrders: boolean;
    drivers?: IDriverMapLocation[];
    isLoadingDrivers: boolean;
    selectedOrder: IDispatcherOrder | null;
    setSelectedOrder: (order: IDispatcherOrder | null) => void;
    selectedDriver: IDriverMapLocation | null;
    setSelectedDriver: (driver: IDriverMapLocation | null) => void;
    radius: number;
                ...
                <CardContent className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {isLoadingOrders ? (
                        ...
                        <div className="text-center text-neutral-400 text-sm py-20 italic flex flex-col items-center justify-center gap-3">
                            <Icon path={mdiInboxRemoveOutline} size={1.2} className="text-neutral-400" />
            ...
            <Card className="bg-darkCardV1 border-darkBorderV1 flex flex-col min-h-0 shadow-2xl">
                ...
                <CardContent className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {!selectedOrder ? (
                        ...
                        <div className="space-y-3">
                            {[...Array(3)].map((_, i) => (
                                <DriverAssignCardSkeleton key={i} />
                            ))}
                        </div>
                    ) : filteredDrivers?.length === 0 ? (
                        <div className="text-center text-neutral-400 text-sm py-20 italic flex flex-col items-center justify-center gap-3">
                            <Icon path={mdiInboxRemoveOutline} size={1.2} className="text-neutral-400" />
                            <p>Không tìm thấy tài xế trong bán kính {radius}km</p>
                        </div>
                    ) : (
...
const OrderCardSkeleton = () => (
    <div className="rounded-2xl border border-darkBorderV1 bg-darkBackgroundV1 flex flex-col shadow-sm">
        <div className="flex justify-between p-3 border-b border-b-darkBorderV1">
        ...
        <div className="p-3 space-y-2">
            <div className="flex gap-2"><Skeleton className="h-4 w-4" /><Skeleton className="h-4 w-full" /></div>
            <div className="flex gap-2"><Skeleton className="h-4 w-4" /><Skeleton className="h-4 w-full" /></div>
        </div>
        <div className="flex justify-between p-3 border-t border-darkBorderV1">
...
const DriverAssignCardSkeleton = () => (
    <div className="rounded-2xl border border-darkBorderV1 bg-darkBackgroundV1 flex flex-col shadow-sm">
        <div className="flex justify-between p-3 border-b border-b-darkBorderV1">
            <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 p-3 bg-darkBackgroundV1/30">
            <Skeleton className="h-6 w-16 rounded-full" />
            <Skeleton className="h-6 w-20 rounded-full" />
